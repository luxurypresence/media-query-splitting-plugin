const getPrefetchOption = ({ prefetch, mediaKeys, mediaKey }) => {
  const prefetchArray = Array.isArray(prefetch)
    ? prefetch
    : prefetch
    ? [prefetch]
    : [];
  const notAllowedPrefetchValues = prefetchArray.filter(
    (prefetchKey) => !mediaKeys.includes(prefetchKey)
  );

  if (notAllowedPrefetchValues.length) {
    console.error(`
      Wrong prefetch values passed to "prefetch" parameter: ${notAllowedPrefetchValues.join(
        ", "
      )} at media "${mediaKey}".
...
const getMediaOptions = (media) => {
  ...
  mediaKeys.forEach((mediaKey) => {
    ...
    if (mediaKey === "common") {
      ...
      mediaValue.query &&
      typeof mediaValue.query === "string"
    ) {
      const {
        query,
        prefetch,
      ...
      console.error(`
        Wrong mediaQuery value "${JSON.stringify(
          mediaValue
        )}" passed to config for media "${mediaKey}",
